    ...
    <div class="container-learning">
        ...
        <div class="video-section">
            <div class="video-title">
                <h1 class="text-2xl font-bold m-3">
                    <?php echo htmlspecialchars($course['title'], ENT_QUOTES, 'UTF-8'); ?></h1>
            </div>
            ...
            <div class="flex items-center justify-between">
                <div class="description" id="video-description">
                    <p class="text-3xl m-5 mb-0">
                        <strong>Bài <?php echo $currentLesson['order_number']; ?>:
                            <?php echo htmlspecialchars($currentLesson['title'], ENT_QUOTES, 'UTF-8'); ?></strong>
                    </p>
                    <p class="mx-5">
                        <?php echo htmlspecialchars($currentLesson['description'] ?? '', ENT_QUOTES, 'UTF-8'); ?></p>
                </div>
                <div class="ghichu" id="ghichu">
                    <!-- Nút Thêm Ghi Chú -->
                    <button id="note-button" class="bg-purple-500 text-white px-4 py-2  rounded mr-5"
                        onclick="openModal()">
            ...
            <div id="questionCountModal"
                ...
                <div class="w-full max-w-md p-6 bg-white rounded-lg shadow-lg">
                    ...
                    <form id="questionCountForm">
                        <div class="mb-4">
                            <label for="questionCount" class="block text-lg font-medium text-gray-700 mb-2">Nhập số câu
                                hỏi (5-20):</label>
                            <input type="number" id="questionCount" name="questionCount" min="5" max="20" value="5"
                                required
                                class="w-full border border-gray-300 rounded-lg p-2 focus:ring focus:ring-blue-300 focus:outline-none"
                                placeholder="Từ 5 đến 20">
                        </div>
                        <div class="flex justify-end gap-2">
                            <button type="button" id="cancelQuestionCountBtn"
                                class="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 transition">Hủy</button>
                            <button type="submit"
                                class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition">Tạo</button>
                        </div>
                    </form>
                </div>
            </div>
            <!-- Phần hiển thị nội dung video -->
            <div class="video-content-section mt-5 p-4 bg-white rounded-lg shadow-lg" id="video-content-container"
                style="display: none;">
                ...
                <form id="quiz-form">
                    ...
                    <div class="mt-6 flex justify-end gap-2">
                        <button type="submit" id="submit-quiz-btn"
                            class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 transition hidden">Nộp
                            bài</button>
                        <button type="button" id="close-quiz-btn"
                            class="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition hidden">Đóng</button>
                    </div>
                </form>
            ...
            function fetchVideoContent(forceReload) {
                ...
                if (!currentLessonId || !currentCourseId) {
                    videoContent.innerHTML =
                        '<li class="text-red-500">Thông tin bài học hoặc khóa học không hợp lệ!</li>';
                ...
                fetch('/generate_video_content', {
                    ...
                    .then(data => {
                        if (data.status === 'success' && data.content) {
                            ...
                            videoContent.innerHTML =
                                `<li class="text-red-500">Không thể tải nội dung video: ${data.message || 'Lỗi không xác định'}</li>`;
                    ...
                    .catch(error => {
                        ...
                        videoContent.innerHTML =
                            '<li class="text-red-500">Lỗi khi kết nối tới server. Vui lòng thử lại.</li>';
                        closeContentBtn.style.display = 'block'; // Hiển thị nút Đóng khi lỗi
                        reloadContentBtn.style.display = 'block'; // Hiển thị nút Tải lại khi lỗi
                    });
            }
            ...
            function displayVideoContent(content) {
                ...
                if (typeof content === 'string') {
                    ...
                    li.innerHTML =
                        `<span class="font-semibold text-blue-600">00:00</span> - <span class="text-gray-800">${content}</span>`;
                    ...
                    content.forEach((item, index) => {
                        ...
                        li.innerHTML =
                            `<span class="font-semibold text-blue-600">${timestamp}</span> - <span class="text-gray-800">${item}</span>`;
                        videoContent.appendChild(li);
                    });
                    ...
                    timestamps.forEach(timestamp => {
                        const li = document.createElement('li');
                        li.innerHTML =
                            `<span class="font-semibold text-blue-600">${timestamp}</span> - <span class="text-gray-800">${content[timestamp]}</span>`;
                        videoContent.appendChild(li);
                    });
                } else {
                    const li = document.createElement('li');
                    li.innerHTML =
                        '<span class="font-semibold text-blue-600">00:00</span> - <span class="text-gray-800">Không có nội dung video để hiển thị.</span>';
                    videoContent.appendChild(li);
                }
            }
            ...
            function generateAIQuestions(questionCount) {
                ...
                if (!currentLessonId || !currentCourseId) {
                    questionsList.innerHTML =
                        '<p class="text-red-500">Thông tin bài học hoặc khóa học không hợp lệ!</p>';
                ...
                fetch('/generate_questions', {
                    ...
                    .then(data => {
                        ...
                        if (data.status === 'success') {
                            ...
                            questionsList.innerHTML = '<p class="text-red-500">Lỗi từ server: ' + (data.message ||
                                'Không thể tạo câu hỏi.') + '</p>';
                            closeQuizBtn.style.display = 'block';
                        }
                    })
                    .catch(error => {
                        console.error('Fetch error:', error);
                        questionsList.innerHTML =
                            '<p class="text-red-500">Lỗi khi kết nối tới server. Vui lòng thử lại.</p>';
                        closeQuizBtn.style.display = 'block';
                    });
            }
            ...
            <div id="noteModal" class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 hidden">
                <div class="w-full max-w-2xl p-4 bg-gray-900 text-white rounded-lg">
                    <div class="mb-2 text-lg">
                        <p id="note-time" class="text-lg mb-3">Thêm ghi chú tại <span
                                class="bg-purple-500 text-white px-2 py-1 rounded">00:00</span></p>
                    </div>
                    <div class="bg-gray-800 p-4 rounded-lg">
                        <div class="flex items-center mb-2">
                            <button onclick="toggleActive(this, 'bold')"
                                class="bg-gray-700 text-white p-2 rounded mr-2"><i class="fas fa-bold"></i></button>
                            <button onclick="toggleActive(this, 'italic')"
                                class="bg-gray-700 text-white p-2 rounded mr-2"><i class="fas fa-italic"></i></button>
                            <button onclick="toggleList(this, 'insertUnorderedList')"
                                class="bg-gray-700 text-white p-2 rounded mr-2"><i class="fas fa-list-ul"></i></button>
                            <button onclick="toggleList(this, 'insertOrderedList')"
                                class="bg-gray-700 text-white p-2 rounded mr-2"><i class="fas fa-list-ol"></i></button>
                            <button onclick="insertImage()" class="bg-gray-700 text-white p-2 rounded mr-2"><i
                                    class="fas fa-image"></i></button>
                            <button onclick="openColorPopup('textColorPopup')"
                                class="bg-gray-700 text-white p-2 rounded mr-2"><i
                                    class="fas fa-paint-brush"></i></button>
                            <button onclick="openColorPopup('bgColorPopup')"
                                class="bg-gray-700 text-white p-2 rounded mr-2"><i
                        ...
                        <form id="noteForm" action="/notes" method="POST">
                            <div id="noteContent" contenteditable="true"
                                class="w-full bg-gray-700 text-white p-2 rounded h-32 overflow-auto"
                                style="outline: none;"></div>
                            <input type="hidden" name="note" id="note">
                            <input type="hidden" id="video_time" name="video_time">
                            <input type="hidden" id="users_id" name="users_id"
                                value="<?php echo isset($users['id']) ? $users['id'] : ''; ?>">
                            <input type="hidden" name="courses_id" value="<?php echo $course['id']; ?>">
                            <input type="hidden" id="lessons_id" name="lessons_id"
                                value="<?php echo $lessons_id = $_POST['lessons_id'] ?? null; ?>">

                            <div class="flex justify-between mt-4">
                                <button type="submit" class="bg-purple-500 text-white px-4 py-2 rounded">TẠO GHI
                                    CHÚ</button>
                                <button type="button" onclick="closeModal()"
                                    class="bg-gray-800 text-white px-4 py-2 rounded">HỦY BỎ</button>
                            </div>
                        </form>
        ...
        <div class="sidebar1">
            <div class="progress-bar mt-4">
                ...
                <br>
                <span id="course-progress">Tiến độ khóa học:
                    <?php echo number_format($progress['progress'], 2); ?>%</span>
                <div class="progress-container">
                    <div id="progress-bar" class="progress-fill" style="width: <?php echo $progress['progress']; ?>%;">
                    </div>
                </div>
                <span id="current-lesson-progress">Tiến độ bài học: 0%</span>
                <div id="certificate-notice" style="display: none; margin-top: 10px;">
                    <p class="text-green-500">Chúc mừng! Bạn đã hoàn thành khóa học. Chứng chỉ đã được gửi qua email của
                        bạn.</p>
                </div>
                <?php if ($certificate): ?>
                <div id="certificate-link" style="margin-top: 10px;">
                    <p class="text-blue-500">Bạn đã hoàn thành khóa học:
                        <a href="http://localhost:8000/certificate?certificate_url=<?php echo htmlspecialchars($certificate['certificate_url'], ENT_QUOTES, 'UTF-8'); ?>"
                ...
                <div id="notes-sidebar"
                    class="fixed top-0 right-0 h-full w-80 bg-white text-black p-4 transform translate-x-full transition-transform duration-300 rounded-l-lg shadow-lg">
                    ...
                    <div class="mb-4 flex items-center">
                        <input type="text" id="sectionFilter" class="bg-gray-200 text-black p-1 rounded-lg w-full mr-2"
                            placeholder="Nhập số phần (ví dụ: 1)">
    ...
    `;
                ...
                function attachEventListeners() {
                    ...
                    document.querySelectorAll('.edit-note').forEach(button => {
                        button.addEventListener('click', function() {
                            ...
                            this.closest('.dropdown-menu').classList.add(
                            'hidden'); // Ẩn dropdown sau khi click
                        });
                    });
                    ...
                    document.addEventListener('click', function(event) {
                        if (!event.target.closest('.dropdown-btn') && !event.target.closest(
                                '.dropdown-menu')) {
                            document.querySelectorAll('.dropdown-menu').forEach(menu => {
                            ...
                            try {
                                ...
                                if (data.status === 'success') {
                                    document.querySelector(
                                            `.note-item[data-id="${noteId}"] .note-text`)
                                        .innerHTML = noteContent;
                                    Swal.fire("Thành công!", "Ghi chú đã được cập nhật!",
                                    "success");
                                    closeEditModal();
                                } else {
                                    Swal.fire("Lỗi!", data.message ||
                                        "Có lỗi khi cập nhật ghi chú.", "error");
                                }
                            } catch (error) {
                    ...
                    <div class="bg-gray-800 p-4 rounded-lg">
                        <form id="editNoteForm">
                            <div class="flex flex-wrap gap-2 mb-3">
                                <button type="button" onclick="toggleActive(this, 'bold')" class="editor-btn"><i
                                        class="fas fa-bold"></i></button>
                                <button type="button" onclick="toggleActive(this, 'italic')" class="editor-btn"><i
                                        class="fas fa-italic"></i></button>
                                <button type="button" onclick="toggleList(this, 'insertUnorderedList')"
                                    class="editor-btn"><i class="fas fa-list-ul"></i></button>
                                <button type="button" onclick="toggleList(this, 'insertOrderedList')"
                                    class="editor-btn"><i class="fas fa-list-ol"></i></button>
                                <button type="button" onclick="insertImage()" class="editor-btn"><i
                                        class="fas fa-image"></i></button>
                                <button type="button" onclick="openColorPopup('textColorPopup')" class="editor-btn"><i
                                        class="fas fa-paint-brush"></i></button>
                                <button type="button" onclick="openColorPopup('bgColorPopup')" class="editor-btn"><i
                                        class="fas fa-fill-drip"></i></button>
                            </div>
                            <div contenteditable="true" id="editNoteContent"
                                class="w-full bg-gray-700 text-white p-3 rounded-lg h-36 overflow-auto border border-gray-600 focus:outline-none focus:ring-2 focus:ring-purple-500">
                            </div>
                            ...
                            <div class="flex justify-end mt-4 gap-2">
                                <button type="button" onclick="closeEditModal()"
                                    class="bg-gray-700 text-white px-4 py-2 rounded-lg hover:bg-gray-600 transition">HỦY</button>
                                <button type="submit"
                                    class="bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-500 transition">LƯU</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div class="course-content">
                ...
                <div class="chapter" onclick="toggleDropdown(this)">
                    <span>Phần <?php echo $section['order_number']; ?>:
                        <?php echo htmlspecialchars($section['title'], ENT_QUOTES, 'UTF-8'); ?></span>
                ...
                <ul class="lesson-list">
                    ...
                    <li class="lesson-item <?php echo $isLocked ? 'locked' : ''; ?>"
                        ...
                        <a href="/courses/learning/<?php echo $course['id']; ?>?course=<?php echo $course['id']; ?>&lesson=<?php echo $lesson['id']; ?>"
                            ...
                            Bài <?php echo $lesson['order_number']; ?>:
                            <?php echo htmlspecialchars($lesson['title'], ENT_QUOTES, 'UTF-8'); ?>
...
<!-- // thêm đoạn này  -->
    ...
    <div class="review-section mt-8 p-4 bg-white rounded shadow ">
        ...
        <form id="review-form" class="space-y-4">
            ...
            <div class="flex space-x-1">
                <?php for ($i = 1; $i <= 5; $i++) : ?>
                <button type="button" id="rating-<?= $i ?>" class="star-button text-gray-400 text-3xl"
                    data-value="<?= $i ?>">★</button>
                <?php endfor; ?>
            ...
            <button type="submit" id="submit-button"
                class="w-full bg-blue-600 text-white py-2 px-4 rounded-lg text-lg font-medium hover:bg-blue-700 transition duration-200">
                Gửi đánh giá
            </button>
        </form>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        form.addEventListener("submit", function(event) {
            ...
            let apiUrl = reviewId ? "/courses/review/update" :
            "/courses/review/add"; // Chọn URL API tùy thuộc vào việc đang thêm hay cập nhật
            ...
            fetch(apiUrl, {
                ...
                .then((data) => {
                    ...
                    if (data.status === "success") {
                        location
                    .reload(); // Nếu thành công, load lại trang để cập nhật danh sách đánh giá
                    }
                })
                .catch((error) => {
                        ...
                        data.reviews.forEach((review) => {
                            ...
                            if (review.images.length > 0) {
                                review.images.forEach((image) => {
                                    imageHtml +=
                                        `<img src="/${image}" class="w-16 h-16 object-cover rounded-md shadow-md mr-2">`;
                                });
                            }

                            let isOwner = review.user_id ==
                                <?= json_encode($_SESSION['user']['id'] ?? null) ?>;
                    ...
                    `;
                        ...
                        document.getElementById("reviews-list").addEventListener("click", function(event) {
                            ...
                            if (event.target.closest(".dropdown-btn")) {
                                let dropdownMenu = event.target.closest(".dropdown-btn")
                                    .nextElementSibling;
                                document.querySelectorAll(".dropdown-menu").forEach(menu => {
                                    if (menu !== dropdownMenu) menu.classList.add("hidden");
                                });
        ...
        if (event.target.classList.contains("edit-review")) {
            ...
            images.forEach((imgSrc) => {
                ...
                removeBtn.onclick = function() {
                    ...
                    document.getElementById("deleted-images").value = JSON.stringify(
                    deletedImages); // Cập nhật lại giá trị input hidden
                };
    ...
    try {
        ...
        window.onYouTubeIframeAPIReady = function() {
            ...
            if (lessonId) {
                ...
                if (activeLesson && !activeLesson.classList.contains('disabled')) {
                    changeVideo(activeLesson.dataset.video, activeLesson.dataset.title, activeLesson.dataset
                        .description, lessonId, activeLesson.dataset.orderNumber);
                } else {
                ...
                if (firstLesson) {
                    changeVideo(firstLesson.dataset.video, firstLesson.dataset.title, firstLesson.dataset
                        .description, firstLesson.dataset.lessonId, firstLesson.dataset.orderNumber);
                } else {
        ...
        function onPlayerStateChange(event) {
            ...
            if (event.data === YT.PlayerState.PLAYING) {
                ...
                progressInterval = setInterval(() => {
                    ...
                    document.getElementById("current-lesson-progress").textContent =
                        `Tiến độ bài học: ${Math.round(progress)}%`;
        ...
        function updateNoteButtonTime() {
            if (player && typeof player.getCurrentTime === 'function') {
                ...
                document.getElementById('note-button').innerHTML =
                    `<i class="fas fa-plus mr-2"></i> Thêm Ghi Chú ${formattedTime}`;
            }
        }
        ...
        function openModal() {
            ...
            document.getElementById('note-time').innerHTML =
                `Thêm ghi chú tại <span class="bg-purple-500 text-white px-2 py-1 rounded">${formattedTime}</span>`;
            ...
            if (currentLessonIndex !== -1) {
                ...
                if (nextLessonIndex < allLessons.length) {
                    ...
                    if (currentSectionId !== nextSectionId) {
                        let currentSectionLessons = document.querySelectorAll(
                            `.lesson-item[data-section-id='${currentSectionId}'] a`);
                        ...
                        if (allCompleted) {
                            let nextSectionLessons = document.querySelectorAll(
                                `.lesson-item[data-section-id='${nextSectionId}']`);
                            nextSectionLessons.forEach(item => {
                                ...
                                lessonLink.onclick = function(e) {
                                    e.preventDefault();
                                    changeVideo(lessonLink.dataset.video, lessonLink.dataset.title,
                                        lessonLink.dataset.description, lessonLink.dataset.lessonId,
                                        lessonLink.dataset.orderNumber);
                                };
                            });
                        ...
                        nextLesson.onclick = function(e) {
                            e.preventDefault();
                            changeVideo(nextLesson.dataset.video, nextLesson.dataset.title, nextLesson.dataset
                                .description, nextLesson.dataset.lessonId, nextLesson.dataset.orderNumber);
                        };
            ...
            fetch('/calculate_progress', {
                ...
                if (data.status === 'success') {
                    ...
                    document.getElementById("course-progress").textContent =
                        `Tiến độ khóa học: ${courseProgress}%`;
                    document.getElementById("progress-bar").style.width = `${courseProgress}%`;

                    let status = courseProgress >= 100 ? 'completed' : (courseProgress > 0 ? 'in-progress' :
                        'enrolled');

...
<form action="" method="POST" class="quiz-form">
    ...
    <select name="question_id" class="input-field">
        ...
        <?php foreach ($questions as $question) : ?>
            <option value="<?= $question['id'] ?>" <?= $question_id == $question['id'] ? 'selected' : '' ?>>
        ...
        <?php if (!empty($answers)) : ?>
            <?php foreach ($answers as $index => $answer) : ?>
                <div class="answer-group">
                    <input type="text" name="answers[]" value="<?= htmlspecialchars($answer) ?>" class="input-field" required>
                    <label class="checkbox-label">
                        <input type="checkbox" name="is_correct[<?= $index ?>]" value="1" <?= isset($is_corrects[$index]) && $is_corrects[$index] == 1 ? 'checked' : '' ?>>
                        Đáp án đúng?
                    </label>
    ...
    <a href="/quizAnswers" class="submit-btn">Trở về quản lý</a>
</form>
